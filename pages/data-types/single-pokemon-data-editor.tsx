...
const DataEditorInner: React.FC = () => {
  ...
  // Holds the progressively enriched Pokemon list
  const [pokemonList, setPokemonList] = useState<Pokemon[]>([]);
  const [detailsLoaded, setDetailsLoaded] = useState(false);
  const abortRef = useRef(false);

  // Phase 1: Fetch the lightweight list instantly (single request)
  const {
    value: rawList,
    loading: loadingList,
    error: listError,
  ...
  // Set the initial lightweight list as soon as it's available
  useEffect(() => {
    if (rawList) {
  ...
  // Phase 2: Progressively batch-load details in the background
  const loadDetails = useCallback(
    async (list: { url: string; id: number; slug: string; name: string }[]) => {
      ...
      for (let i = 0; i < list.length; i += BATCH_SIZE) {
        ...
        const enriched = await Promise.all(
          batch.map(async (pokemon) => {
            try {
              ...
              // On failure, return the lightweight version as-is
              return pokemon;
            }
          })
        );
        ...
        // Merge this batch into the list
        setPokemonList((prev) => {
          const updated = [...prev];
  ...
  const selectedSlugs = id ? [id] : [];
  const selectedDetails = pokemonList.find((p) => p.slug === id);
  ...
  return (
    <PokemonSelector
      ...
      selectedPokemonDetails={selectedDetails}
